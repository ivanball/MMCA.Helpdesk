...
/// is exactly what the AppHost's <c>WithSQLServerDataSource(helpdeskDb, "Tickets")</c> injects at run
/// time, and the resolver collapses a logical name onto <c>Default</c> when the two connections match.
/// The scaffolded model therefore matches the running one, which matters for the framework tables that
/// are Default-source-only (<c>ScheduledJobs</c>). Point <c>HELPDESK_TICKETS_SQL</c> at a real server
/// to apply.
/// </remarks>
